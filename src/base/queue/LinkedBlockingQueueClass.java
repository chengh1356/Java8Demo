...
/**
 * @Description 基于链表的无界阻塞队列，同ArrayBlockingQueue类似
 *              内部采用分离锁，实现生产者和消费者操作完全并行
 * @Author cgh
 * @Date 2020-04-22 下午 3:30
 */
public class LinkedBlockingQueueClass {
    ...
    static class ProviderThread extends Thread {
        ...
        public void run() {
            ...
            // 本线程处理完毕的标记
            LinkedBlockingQueueClass.providerDoneFlag[threadNo] = true;
    ...
    static class ConsumerThread implements Runnable {
        ...
        public void run() {
            ...
            while (LinkedBlockingQueueClass.getDone() == false || linkedBlockingQeque.isEmpty() == false) {
                try {
    ...
    public static synchronized void setDone(boolean flag) {
        LinkedBlockingQueueClass.done = flag;
    }

    public static synchronized boolean getDone() {
        return LinkedBlockingQueueClass.done;
    }
    ...
    public static void main(String[] args) {
        ...
        do {
            for (boolean b : providerDoneFlag) {
                ...
                LinkedBlockingQueueClass.setDone(true);
            }

            // 生产者线程全部结束的时候，跳出检测
            if (LinkedBlockingQueueClass.getDone() == true) {
                break;
            }
        } while (true);
